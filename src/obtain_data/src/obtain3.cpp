...
int32_t present_position[JN][MAX_DATA];
...
// float Fc_1[] = {0.0066, 0.1263, 0.1451, 0.1602, 0.1654, 0.1689, 0.1748};
// float Fc_2[] = {0.0284, 0.2152, 0.2229, 0.2538, 0.2826, 0.2913, 0.2996};
// float Fc_3[] = {0.0055, 0.3289, 0.3396, 0.3725, 0.4169, 0.5341, 0.5826};
...
int main(int argc, char *argv[])
{
    ...
    for (j = 0; j < MAX_DATA; j++)
    {
        ...
        for (i = 0; i < 7; i++)
        {
            ...
            {
                RCLCPP_INFO(rclcpp::get_logger("obtain_data_node"), "[ID:%03d] groupSyncRead getdata position failed", dxl_id[i]);
                return 0;
            }
        }
        ...
        for (i = 0; i < 7; i++)
        {
            ...
            if (dxl_getdata_result != true)
            {
                RCLCPP_INFO(rclcpp::get_logger("obtain_data_node"), "[ID:%03d] groupSyncRead getdata current failed", dxl_id[i]);
                return 0;
            }
        }
        ...
        //RCLCPP_INFO(rclcpp::get_logger("obtain_data_node"), "[Time: %.05lf] [I: %d] [Present Pos Joint 0: %g] [Current Joint 0: %.03lf mA]", time_taken, j, (double)(present_position[0][j], 0, 4096, 0, 360), (int16_t)present_current[1][j] * 2.69);
        RCLCPP_INFO(rclcpp::get_logger("obtain_data_node"), "[Time: %.05lf] [I: %d] [Target Pos Joint 0: %g] [Present Pos Joint 0: %g] [Current Joint 0: %.03lf mA]", time_taken, j, th[0][j], map_range((double)present_position[0][j], 0, 4096, 0, 360) * (M_PI / 180), (int16_t)present_current[1][j] * 2.69);
        ...
        data << time_taken << ","
             ...
             << th[5][j] * (M_PI / 180) << ","
             << th[6][j] * (M_PI / 180) << ","
             << map_range((double)present_position[0][j], 0, 4096, -180, 180) * (M_PI / 180)<< ","
             << map_range((double)present_position[1][j], 0, 4096, -180, 180) * (M_PI / 180)<< ","
             << map_range((double)present_position[2][j], 0, 4096, -180, 180) * (M_PI / 180)<< ","
             << map_range((double)present_position[3][j], 768, 2048, -180, 180) * (M_PI / 180)<< ","
             << map_range((double)present_position[4][j], 0, 4096, -180, 180) * (M_PI / 180)<< ","
             << map_range((double)present_position[5][j], 0, 4096, -180, 180) * (M_PI / 180)<< ","
             << map_range((double)present_position[6][j], 0, 4096, -180, 180) * (M_PI / 180)<< ","
